  ...
  return (
    ...
    >
      <AccordionTrigger className="min-h-[52px] gap-[14px] py-[15px] transition-[padding-left,color] [@media(hover:hover)]:hover:pl-1.5 [@media(hover:hover)]:hover:text-accent">
        <span className="shrink-0 font-mono text-[12px] text-ink-faint tabular-nums">{num}</span>
        <span className="flex min-w-0 flex-1 flex-col gap-[3px]">
          <b className="text-[16.5px] font-semibold tracking-[-0.01em] text-ink max-[560px]:text-[16px]">
            {title}
          </b>
          <em className="text-[14px] text-ink-soft not-italic max-[560px]:text-[13.5px]">
            {subtitle}
          </em>
        </span>
        {badge ? (
